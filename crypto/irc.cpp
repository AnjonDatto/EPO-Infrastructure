...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #epocoinTEST2\r");
            Send(hSocket, "WHO #epocoinTEST2\r");
        } else {
            // randomly join #epocoin00-#epocoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #epocoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #epocoin%02d\r", channel_number).c_str());
        }
