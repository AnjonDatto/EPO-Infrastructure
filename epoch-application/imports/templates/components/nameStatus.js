import { registrar, network } from '/imports/lib/epoereum';
        ...
        if(!err && entry) {
          ...
          if (entry.name) {
            ...
            if (window.location.hash !== '#' + name) {
              history.pushState(null, entry.name + '.epo', '#'+entry.name);
            }
          ...
          if (timeoutName !== name){
            ...
            timeout = setTimeout(function() {
              if (name === Session.get('searched')) {
                var value = entry.mode == 'owned' ? Math.max(Number(web3.fromWei(entry.value.toFixed(), 'epoer')), 0.01) : 0;
                ...
                Names.upsert({name: name}, {$set: {
                  ...
                  deedBalance: Number(web3.fromWei(entry.deed.balance, 'epoer'))
                }});
              }
...
Template['aside-forbidden-can-invalidate'].helpers({
  value() {
    ...
    return web3.fromWei(val ? val.toFixed() : 0, 'epoer');
  },
  invalidatorFee() {
    var val = Template.instance().data.entry.deed.balance;
    return web3.fromWei(val ? val.toFixed()/2 : 0, 'epoer');
  },
  hasNode() {
    return LocalStore.get('hasNode');
  }
...
Template['status-reveal'].events({
  'click .import-bids': function(e) {
    epoElements.Modal.show('modals_restore');
    e.preventDefault();
  }
})
