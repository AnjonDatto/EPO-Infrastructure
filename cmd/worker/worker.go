...
import (
	...
	"github.com/epoledger/sandbox/job"

	"cloud.google.com/go/pubsub"
	"github.com/epoledger/gepo"
	"go.uber.org/zap"
...
func (app *App) processTransactions(ctx context.Context, attachReq *gepo.AttachToTangleRequest) ([]gepo.Transaction, error) {
	ctx.Err()

	outTxs := []gepo.Transaction{}
	var prevTxHash *gepo.Trytes = nil

	for _, ts := range attachReq.Trytes {
		trits := ts.Trytes().Trits()
		if prevTxHash == nil {
			copy(trits[gepo.TrunkTransactionTrinaryOffset:gepo.TrunkTransactionTrinaryOffset+gepo.TrunkTransactionTrinarySize], attachReq.TrunkTransaction.Trits())
			copy(trits[gepo.BranchTransactionTrinaryOffset:gepo.BranchTransactionTrinaryOffset+gepo.BranchTransactionTrinarySize], attachReq.BranchTransaction.Trits())
		} else {
			copy(trits[gepo.TrunkTransactionTrinaryOffset:gepo.TrunkTransactionTrinaryOffset+gepo.TrunkTransactionTrinarySize], prevTxHash.Trits())
			copy(trits[gepo.BranchTransactionTrinaryOffset:gepo.BranchTransactionTrinaryOffset+gepo.BranchTransactionTrinarySize], attachReq.TrunkTransaction.Trits())
		}
		...
		outS := string(out)
		outTx, err := gepo.NewTransaction(gepo.Trytes(outS))
		if err != nil {
...
func (app *App) HandleAttachToTangle(ctx context.Context, j *job.IRIJob) error {
	...
	j.AttachToTangleRespose = &gepo.AttachToTangleResponse{Trytes: outTxs}
