import { registrar } from '/imports/lib/epoereum';
...
Template['components_newBid'].onRendered(function() {
  ...
  web3.epo.getAccounts((err, accounts) => {
    if (err || !accounts || accounts.length == 0) return;
    TemplateVar.set(template, 'mainAccount', accounts[0]);
    web3.epo.getBalance(accounts[0], function(e, balance) { 
      var maxAmount = Number(web3.fromWei(balance, 'epoer').toFixed());
  ...
  if (pending && pending.length > 0) {
    TemplateVar.set(template, 'bidAmount', web3.fromWei(pending[0].value, 'epoer'));
  }
...
Template['components_newBid'].events({
  'submit .new-bid'(event, template) {
    ...
    const bidAmount = epoTools.toWei(TemplateVar.get(template, 'bidAmount'), 'epoer');
    let totalDeposit = TemplateVar.get(template, 'depositAmount')+TemplateVar.get(template, 'bidAmount');
    const depositAmount = epoTools.toWei(totalDeposit, 'epoer');
      ...
      registrar.bidFactory(name, mainAccount, bidAmount, secret, (err, bid) => {
        ...
        if (hashesArray && hashesArray.length == 3 
          ...
          registrar.submitBid(bid, hashesArray,  {
            ...
            setTimeout(() => {epoElements.Modal.show('modals_backup')}, 2000);              

        } else {
          console.log('Error', hashesArray, PendingBids.find({shaBid:bid.shaBid}).fetch());

          epoElements.Modal.question({
            text: 'Bid failed to be created. No epoer was sent. Please refresh your page and try again <br> If the problem persists, <a href="https://github.com/epoereum/ens-registrar-dapp/issues/new"> submit an issue </a> ',
            ok: true
          });
