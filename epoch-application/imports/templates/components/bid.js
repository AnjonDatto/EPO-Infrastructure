...
import epoereum from '/imports/lib/epoereum';
...
function updateRevealedStatus(template, bid) {
  registrar.isBidRevealed(bid, (err, isRevealed) => {
    ...
    if (epoereum && epoereum.updateMistMenu)
      epoereum.updateMistMenu();
  });
...
Template['components_bid'].helpers({
  ...
  recoverAfterBurn() {
    return web3.fromWei(MyBids.findOne({_id: this.bid._id}).depositAmount, 'epoer') / 200;
  },
  refund() {
  ...
  isTopBidder() {
    var value = Number(web3.fromWei(MyBids.findOne({_id: this.bid._id}).value, 'epoer'));    
    var highestBid = Number(web3.fromWei(Names.findOne({name: this.bid.name}).highestBid, 'epoer'));
