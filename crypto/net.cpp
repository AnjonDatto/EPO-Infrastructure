...
static const char *strDNSSeed[][2] = {
    {"node.epocoin.karelia.pro", "dnsseed.epocoin.karelia.pro"},
    {"epocoin.ru", "dnsseed.epocoin.ru"},
    {"epocoin.ru", "testseed.epocoin.ru"},
    {"epoco.in", "dnsseed.epoco.in"},
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("epocoin-dnsseed");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. epoCoin is probably already running."), addrBind.ToString().c_str());
        else
