import { ens, registrar, network } from '/imports/lib/epoereum';
...
Template['status-owned'].helpers({
  ...
  refund() {
    ...
    return web3.toWei(entry.deedBalance - entry.value, 'epoer');
  },
  canRefund() {
    var entry = TemplateVar.get('entryData');
    if (!entry) return false;    
...
Template['status-owned'].events({
  ...
  'click .claim': function(e, template) {
    ...
    reverseRegistrar.setName(name+'.epo', { from: owner, gas: 300000 },
      Helpers.getTxHandler({
        onSuccess: () => GlobalNotification.warning({
